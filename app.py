...
from models import db, Articulos, User, Categorias, Tag
...
def blog_articulo(id):
    ...
    # selct * from user where id = mi_articulo.user_id
...
def admin_login():
    if request.method == 'POST':
        ...
        if my_user and check_password_hash(my_user.password,request.form['password']):
            ...
            flash('Email o password no válidos', 'danger')
...
def admin_nuevo_articulo():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return flash('No se ha subido la imagen de portada', 'danger')
        ## Ejecutamos SQL
        db.session.add(nuevo_articulo)
        db.session.commit()

        # Mostramos una informacion al usuario
...
@app.route("/admin/articulos/editar/<int:id>", methods=['GET', 'POST'])
def admin_editar_articulo(id):
    # Protegemos nuestra pagina
    if 'id' not in session:
        flash('No tienes acceso', 'danger')
        return redirect(url_for('blog_all'))
    ...
    tags = Tag.query.all()
    return render_template(
        'web/admin/editar_articulo.html', 
        categorias=categorias,
        tags=tags,
        articulo=mi_articulo
    )
...
def borrar_categoria():
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        # Marcamos la categoría por defecto para los artículos
...
@app.route("/admin/tags/nuevo", methods=['GET', 'POST'])
def admin_nuevo_tag():
    # Protegemos nuestra pagina
    if 'id' not in session:
        flash('No tienes acceso', 'danger')
...
@app.route("/admin/tags/editar/<int:id>", methods=['GET', 'POST'])
def admin_editar_tag(id):
    # Protegemos nuestra pagina
    if 'id' not in session:
        flash('No tienes acceso', 'danger')
        return redirect(url_for('blog_all'))
...
def buscar():
    if request.args.get('q'):
        ...
        # Si no hay una búsqueda, lo tiramos de nuevo al blog
        return redirect(url_for('blog_all'))
